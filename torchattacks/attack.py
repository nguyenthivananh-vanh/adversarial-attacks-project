...
class Attack(object):
    ...
    # lay ra cac diem so cua mo hinh cho cac hinh anh dau vao
    def get_logits(self, inputs, labels=None, *args, **kwargs):
        if self._normalization_applied is False:
            # neu chua duoc chuan hoa thi chuan hoa
...
#                 logging.info("Normalization automatically loaded from `model.mean` and `model.std`.")
    ...
    def normalize(self, inputs):
        mean = self.normalization_used['mean'].to(inputs.device) # gia tri TB
        std = self.normalization_used['std'].to(inputs.device) # do lech chuan
